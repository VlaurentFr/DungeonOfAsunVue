...
const selected = computed(() => {
  return dons.filter(x => 
    x.nom.toLowerCase().includes(search.value.toLowerCase())
...
<template>
  <div class='visible'>
    <div class='header'>
      <h1>Les Dons</h1>
      <h2>Découvrez la liste des<span> dons </span></h2>
      <div id="search">
        <i class="fas fa-search"></i>
        <input placeholder="Rechercher une compétence ..." type="text" v-model="search"/>
        <div v-if="selected.length !== dons.length" class="searh-suggest">
          <div class="suggest" v-for="m of selected" :key="m.nom" @click="clickSuggest(m.nom)">
            <p>
              {{ m.nom }}
            </p>
          </div>
    ...
    <div class="cards">
      <div class="card" v-for="s of selected" :key="s.nom" :id="s.nom">
        <div class="card-desc">
          <h4>
            {{ s.nom }}
          </h4>
        ...
        <div class="side">
          <p><span>Prerequis :</span> {{ s.prerequis }}</p>
        </div>
      </div>
    </div>
  </div>
